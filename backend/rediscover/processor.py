...
class ReDiscoverV2Processor:
    ...
    def _analyze_target_period(self, target_tracks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_genres": dict(
                sorted(genre_counts.items(), key=lambda item: item[1], reverse=True)[:5]
            ),
            "top_artists": dict(
                sorted(artist_counts.items(), key=lambda item: item[1], reverse=True)[:5]
            ),
            "top_decades": dict(
    ...
    async def _create_playlist_data(
        ...
        user_id: str,
        server_id: str,
        used_smart_playlist: bool = False,
    ) -> Dict[str, Any]:
    ...
    async def _log_to_database_v2(
        ...
        print(
            f"📊 V2 Playlist logged: {len(playlist_data['tracks'])} tracks, "
            f"theme: {theme_strategy.get('theme_identified', 'Unknown')}, "
            f"tracks analyzed: {tracks_analyzed}"
        )
