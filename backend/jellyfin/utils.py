...
def _normalize_genres(genres_raw: Any) -> List[str]:
    """Normalize genre input (handle various Jellyfin formats)"""
    if not genres_raw:
        return []
    
    if isinstance(genres_raw, list):
        return [g.strip() for g in genres_raw if g and g.strip()]
    
    if isinstance(genres_raw, str):
        return _parse_genre_string(genres_raw)
