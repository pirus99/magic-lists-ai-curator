...
The public entry point ``filter_tracks_for_this_is_playlist`` is re-exported here so
existing ``from .track_scoring import filter_tracks_for_this_is_playlist`` imports keep
working. The implementation is split into ``engagement`` (scoring/threshold) and
``diversity`` (artist/album caps) helper modules.
"""
from .filtering import filter_tracks_for_this_is_playlist
